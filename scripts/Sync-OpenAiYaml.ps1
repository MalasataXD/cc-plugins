...
function Get-ExistingDisplayName([string]$yamlPath) {
    ...
    if ((Get-Content $yamlPath -Raw) -match 'display_name:\s*"(.*)"') { return $Matches[1] }
    return $null
}
...
foreach ($skillMd in Get-ChildItem $skills -Filter SKILL.md -Recurse -Depth 2 | Sort-Object FullName) {
    ...
    $yamlPath = Join-Path $skillMd.DirectoryName 'agents' 'openai.yaml'
    $name = (Get-ExistingDisplayName $yamlPath) ?? (Get-DisplayName $meta['name'])
    $userInvoked = ($meta['disable-model-invocation'] ?? '').ToLower() -eq 'true'
    $wanted = Format-Yaml $name $meta['description'] $userInvoked
    $current = if (Test-Path $yamlPath) { (Get-Content $yamlPath -Raw).Replace("`r`n", "`n") } else { '' }
    if ($current -eq $wanted) { continue }

    $drift += [System.IO.Path]::GetRelativePath($root, $skillMd.DirectoryName)
    if (-not $Check) {
