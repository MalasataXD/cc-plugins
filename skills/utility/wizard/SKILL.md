...
**Never give a time estimate.** No minutes in the script, in the stage headers, or in what you tell the user — not for a stage, not for the wizard as a whole. Progress is measured in stages only.
...
When an **RFH ticket**'s human-only steps are this kind of manual procedure — credentials, dashboards, provisioning — offer a wizard for exactly those steps instead of dumping numbered instructions into the chat. The stage-list confirmation below doubles as the plan's "Where I'll need you" section.
